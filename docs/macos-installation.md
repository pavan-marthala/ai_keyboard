...
This guide covers system requirements, installation steps, Gatekeeper verification, and required system permissions for running **AI Keyboard** on macOS.
...
> AI Keyboard is currently distributed as an unnotarized, open-source macOS application. The project does **not** currently have an active Apple Developer Program subscription. Consequently:
>
...
1. Download the latest `ai_keyboard-macos.zip` or `.app` bundle from the official [GitHub Releases](https://github.com/pavan-marthala/ai_keyboard/releases) page.
2. If downloaded as a ZIP archive, extract it.
3. Drag `ai_keyboard.app` into your `/Applications` directory:

   ```bash
   mv ~/Downloads/ai_keyboard.app /Applications/
   ```
...
cp -R build/macos/Build/Products/Release/ai_keyboard.app /Applications/
```
...
> *"Apple could not verify 'ai_keyboard' is free of malware that may harm your Mac or compromise your privacy."*  
> or  
> *"'ai_keyboard' cannot be opened because it is from an unidentified developer."*

This occurs because the browser marks the downloaded file with a macOS quarantine attribute (`com.apple.quarantine`).

> [!WARNING]
> Only bypass Gatekeeper if you obtained `ai_keyboard` directly from the official [GitHub repository](https://github.com/pavan-marthala/ai_keyboard) or compiled it yourself from source.  
...
After moving `ai_keyboard.app` to `/Applications`, open Terminal (`/Applications/Utilities/Terminal.app`) and run:

```bash
xattr -dr com.apple.quarantine /Applications/ai_keyboard.app
```
...
Once executed, you can launch `ai_keyboard` normally from Launchpad, Spotlight, or `/Applications`.
...
2. Locate **ai_keyboard.app**.
3. **Right-click** (or hold <kbd>Control</kbd> and click) on **ai_keyboard.app** and select **Open**.
...
5. *Note for macOS Ventura, Sonoma, and Sequoia:* If the dialog only offers "Move to Trash" or "Cancel", open **System Settings > Privacy & Security**, scroll down to the **Security** section where you will see:
   > *"ai_keyboard was blocked from use because it is not from an identified developer."*
...
To provide system-wide text transformation across external applications, `ai_keyboard` requires two macOS permissions. The app includes a built-in onboarding screen on first launch that checks these statuses and provides direct shortcut buttons to the respective System Settings panes.

### 1. Accessibility (`AXIsProcessTrusted`)

- **Why it is needed:** Enables `ai_keyboard` to inspect the focused text area in external apps (such as TextEdit, Safari, Mail, Slack, Chrome, or Notes), determine the active selected text, and execute the text replacement via simulated keystrokes.
- **How to grant manually:**
  1. Open **System Settings > Privacy & Security > Accessibility**.
  2. Locate **ai_keyboard** in the list and toggle the switch **ON**.
  3. If prompted, authenticate with your Mac user password or Touch ID.
  4. If `ai_keyboard` is not listed, click the **+** (Add) button, navigate to `/Applications`, and select `ai_keyboard.app`.

### 2. Input Monitoring (`IOHIDRequestAccess`)

- **Why it is needed:** Enables `ai_keyboard` to register the global shortcut (<kbd>Control</kbd> + <kbd>Option</kbd> + <kbd>Space</kbd>) via `CGEvent` session taps, allowing you to invoke the AI transformation prompt from any application without switching windows.
- **How to grant manually:**
...
6. Once complete, `ai_keyboard` smoothly refocuses the target app and replaces the selected text with the AI-transformed result.
